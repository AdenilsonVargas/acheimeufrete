...
/**
 * Modal de Avaliação de Transportador
 * 
 * Props:
 * - onClose: função para fechar modal
 * - cotacaoId: ID da cotação
 * - transportadorNome: nome do transportador
 * - onAvaliacaoEnviada: callback após enviar
 * - apiClient: instância do axios
 */

const AvaliarTransportador = ({ onClose, cotacaoId, transportadorNome, onAvaliacaoEnviada, apiClient }) => {
  const [nota, setNota] = useState(0);
  const [notePontualidade, setNotePontualidade] = useState(0);
  const [noteComunicacao, setNoteComunicacao] = useState(0);
  const [noteQualidade, setNoteQualidade] = useState(0);
  ...
  const handleEnviar = async (e) => {
    ...
    try {
      const response = await apiClient.post('/api/avaliacoes/transportador', {
        cotacaoId,
        nota,
        pontualidade: notePontualidade || null,
        comunicacao: noteComunicacao || null,
        qualidadeServico: noteQualidade || null,
        comentario
      });
        ...
        <form onSubmit={handleEnviar} className="p-6 space-y-6">
          ...
          <div className="space-y-4 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
            ...
            <div>
              ...
              {renderStars(notePontualidade, setNotePontualidade)}
            </div>
            ...
            <div>
              ...
              {renderStars(noteComunicacao, setNoteComunicacao)}
            </div>
            ...
            <div>
              ...
              {renderStars(noteQualidade, setNoteQualidade)}
            </div>
          </div>
          ...
          <div className="p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
            <p className="text-xs text-blue-700 dark:text-blue-300">
              <strong>💡 Dica:</strong> Seja honesto e construtivo. Suas avaliações ajudam a melhorar a qualidade do serviço.
            </p>
          </div>
...
export default AvaliarTransportador;
