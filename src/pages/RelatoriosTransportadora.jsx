...
export default function RelatoriosTransportadora() {
  ...
  const fetchCotacoes = async () => {
    setLoading(true);
    try {
      const response = await api.entities.cotacao.list({ limit: 1000 });
  ...
  const exportarCSV = () => {
    const headers = [
      ...
      const arrFiltradas = Array.isArray(cotacoesFiltradas) ? cotacoesFiltradas : [];
      const rows = arrFiltradas.map(c => [
    ...
    link.download = `relatorio_transportadora_${new Date().getTime()}.csv`;
    link.click();
  };
  ...
  const exportarPDF = () => {
    ...
    const html = `
      ...
      <html>
      <head>
        <title>Relatório de Cotações - Transportadora</title>
        <style>
      ...
      <body>
        <h1>Relatório de Cotações - Transportadora</h1>
        <p>Gerado em: ${new Date().toLocaleString('pt-BR')}</p>
        <p>Transportadora: ${user?.nome || 'N/A'}</p>
        
        <div class="stats">
          <div class="stat-card">
            <div>Total de Cotações</div>
            <div class="stat-value">${Array.isArray(cotacoesFiltradas) ? cotacoesFiltradas.length : 0}</div>
          </div>
          <div class="stat-card">
            <div>Valor Total</div>
          ...
          <div class="stat-card">
            <div>Pendentes</div>
            <div class="stat-value">${Array.isArray(cotacoesFiltradas) ? cotacoesFiltradas.filter(c => ['aberta', 'aguardando'].includes(c.status)).length : 0}</div>
          </div>
          <div class="stat-card">
            <div>Finalizadas</div>
            <div class="stat-value">${Array.isArray(cotacoesFiltradas) ? cotacoesFiltradas.filter(c => c.status === 'finalizada').length : 0}</div>
          </div>
        </div>
        ...
        <table>
          ...
          <tbody>
            ${Array.isArray(cotacoesFiltradas) ? cotacoesFiltradas.map(c => `
              <tr>
                <td>${c.id?.slice(0, 8) || c.numero || 'N/A'}</td>
                <td>${c.destinatarioNome || c.destinatario?.nome || 'N/A'}</td>
                <td>${c.embarcadorNome || c.cliente?.nome || c.embarcador?.nome || 'N/A'}</td>
      ...
      <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-orange-400">
                  {Array.isArray(cotacoesFiltradas) ? cotacoesFiltradas.filter(c => ['aberta', 'aguardando', 'em_analise'].includes(c.status)).length : 0}
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-blue-400">
                  {Array.isArray(cotacoesFiltradas) ? cotacoesFiltradas.filter(c => c.status === 'finalizada').length : 0}
                </p>
              </div>
        ...
        <div className="card">
          <h2 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
            ...
            Resultados ({Array.isArray(cotacoesFiltradas) ? cotacoesFiltradas.length : 0} cotações)
          </h2>

          {loading ? (
            <p className="text-slate-300 text-center py-8">Carregando...</p>
          ) : !Array.isArray(cotacoesFiltradas) || cotacoesFiltradas.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-slate-700">
                  {Array.isArray(cotacoesFiltradas) && cotacoesFiltradas.map((cotacao) => (
                ...
                <tfoot className="bg-slate-700">
                  <tr>
                    <td colSpan="4" className="p-3 text-white font-bold">
                      TOTAL DO PERÍODO FILTRADO
                    </td>
                    <td className="p-3 text-right text-green-400 font-bold text-lg">
                      R$ {cotacoesFiltradas.reduce((sum, c) => sum + (parseFloat(c.valor) || 0), 0).toFixed(2)}
                    </td>
